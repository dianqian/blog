...
func (a *ArticleController) Get() {
    ...
    a.TplName = "visitor/article.html"
    return
}
...
/**
 读取article的信息
 */
func (a *ArticleController) setArticle(ar *ArticleInfo) (*models.Article, error) {
    originAr := &models.Article{Url: ar.Slug}
...
/**
 设置prev/next内容
 */
func (a *ArticleController) setBrother(ar *ArticleInfo, originAr *models.Article) error {

    // prev
    tmpAr := &models.Article{}
...
/**
 todo: 设置tag内容
 */
func (a *ArticleController) setTags(ar *ArticleInfo, originAr *models.Article) error {
...
/**
 设置topic内容
 */
func (a *ArticleController) setTopic(ar *ArticleInfo, originAr *models.Article) error {

    tp := &models.Topic{Id: originAr.Id}
    err := tp.Read("id")
    if err != nil {
        return fmt.Errorf("the id='%d no find topic object: %s", originAr.Id, err.Error())
    }

    arTp := &models.ArticleTopic{TopicId: tp.Id}
    ...
    for _, item := range arTps {
        tmpAr := &models.Article{Id: item.ArticleId}
        err = tmpAr.Read("id")
        if err != nil {
