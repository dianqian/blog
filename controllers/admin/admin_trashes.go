...
/**
 ...
 @Return：
 */
func (d *TrashManageController) dealTrashInfo(ars []*db.Article) ([]*htmladmin.ArticleTrashInfo, error) {
    var titles []*htmladmin.ArticleTrashInfo

    for _, item := range ars {
        title := htmladmin.ArticleTrashInfo{ID: item.Id, Title: item.Title}
...
/**
 ...
 @Return：
 */
func (d *TrashManageController) Get ()  {
    // 基础调用函数
    d.AdminBase()
    ...
    arData := htmladmin.HTMLArticleTrashData{}
    // 获取数据
    article := new(db.Article)
    count, err := article.Count(common.ARTICLE_STATUS_DELETE)
    if err != nil {
        logMsg := fmt.Sprintf("select count article for trash error: %s", err.Error())
        logs.Error(logMsg)

        arData.ErrorInfo = logMsg
        d.Data["HTMLArticleTrashData"] = arData
        d.TplName = "admin/admin_trashes.html"
        return
    }
    ars, err := article.Select(offset, limit, common.ARTICLE_STATUS_DELETE)
    if err != nil {
    ...
    trashes, err := d.dealTrashInfo(ars)
    if err != nil {
